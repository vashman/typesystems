...
set_typebuffer<int const, std::vector<int> >(con);
set_typebuffer<std::string, std::vector<std::string> >(con);

/* fill the infomation directly */
typebuffer_interface<int const> & buf1 = use_typebuffer<int const>(con);
buf1.push(24);
buf1.push(32);
buf1.push(42);

/* add only 2 items so the extra int will be ignored */
typebuffer_interface<std::string> & buf2
                    = use_typebuffer<std::string>(con);
buf2.push("probi");
buf2.push("otic");
